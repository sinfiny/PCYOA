...
export function evaluateRequirement(requirement: Requirement, context: RequirementContext): RequirementResult {
    switch (requirement.kind) {
        ...
        case 'selection-count': {
            const selectedCount = getSelectionCount(context.state, context.index, context.cyoa, requirement.choiceIds, requirement.rowIds, requirement.groupIds);
            return toRequirementResult(
                compareCount(selectedCount, requirement.operator, requirement.count),
                `Selection count must be ${requirement.operator} ${requirement.count}.`
            );
        }
        case 'global': {
        ...
        case 'when': {
            const condition = evaluateRequirements(requirement.when, context);
            return condition.satisfied ? evaluateRequirement(requirement.then, context) : { satisfied: true, reasons: [] };
        }
        case 'all':
        ...
        case 'unsupported':
            return toRequirementResult(false, requirement.message, requirement.sourceId);
    }
}
...
export function getSelectionCount(
    ...
    const selectedChoices = choiceIds.reduce((count, choiceId) => count + getChoiceSelectionCount(state, choiceId), 0);
    const selectedRows = rowIds.reduce((count, rowId) => count + getRowSelectionCount(state, index, rowId), 0);
    const selectedGroups = groupIds.reduce((count, groupId) => count + getGroupSelectionCount(state, cyoa, groupId), 0);

    return selectedChoices + selectedRows + selectedGroups;
}
